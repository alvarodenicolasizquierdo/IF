  ...
  return (
    <section className="flex min-h-0 flex-col rounded-xl border border-hairline bg-surface/80">
      <header className="flex items-center gap-2 border-b border-hairline/70 px-4 py-3">
        ...
        <h2 className="text-[13px] font-bold uppercase tracking-[0.14em] text-ink-faint">
          Immutable audit trail
        </h2>
        <span className="ml-auto font-mono text-[13px] text-ink-faint">{entries.length}</span>
      </header>

      <div className="scrollbar-thin min-h-0 flex-1 overflow-y-auto px-3 py-2">
        {entries.length === 0 ? (
          <p className="px-1 py-3 text-[14px] leading-relaxed text-ink-faint">
              ...
              return (
                <li key={entry.id} className="flex gap-2 rounded px-1 py-1">
                  ...
                  <div className="min-w-0">
                    <p className="flex items-center gap-1.5 font-mono text-[12px] uppercase tracking-wider text-ink-faint">
                    ...
                    <p className="mt-0.5 text-[14px] leading-snug text-ink-muted">{entry.message}</p>
                  </div>
                </li>
              );
            })}
