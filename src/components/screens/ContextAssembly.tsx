...
/**
 * Both panels are built the same way: a scrolling body and a pinned action.
 * The Mandate signature is the beat the whole demo turns on, so it stays on
 * screen at every viewport rather than sitting below the fold at 100% zoom.
 */
export function ContextAssembly() {
  ...
  // The findings are the beat, and the panel scrolls, so bring them to the
  // presenter rather than making them hunt for what they just triggered.
  const findingsRef = useRef<HTMLDivElement>(null);
  useEffect(() => {
    if (blockedByHygiene) {
      findingsRef.current?.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
    }
  }, [blockedByHygiene]);
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 gap-5 lg:grid-cols-2">
        ...
        >
          <div className="min-h-0 flex-1 overflow-y-auto pr-1">
            ...
            <div className="mt-3 flex items-center gap-2 rounded-lg border border-hairline bg-canvas/50 px-3 py-2">
            ...
            <div className="mt-4 border-t border-hairline pt-3">
                ...
                <div className="mt-3 flex items-start gap-3 rounded-lg border border-trust-passed/50 bg-trust-passed/10 px-3 py-2.5">
                  ...
                  <div>
                    ...
                    <p className="mt-0.5 text-[14px] leading-relaxed text-ink-muted">
                      Data Product Factory re-indexed the tax schema and classified the billing extract.
                      Context freshness restored to 97%. Sovereign routing override released.
                    </p>
                  </div>
                </div>
              )}
              ...
              {blockedByHygiene && (
                <div ref={findingsRef} className="mt-3 space-y-2.5">
                      ...
                      >
                        <div className="flex items-start gap-2.5">
                          ...
                          <div className="min-w-0">
                            <p className={cx('font-mono text-[14px] font-bold', t.text)}>
                              {finding.artefact}
                            </p>
          ...
          {/* Pinned: the way out of the hygiene block is never scrolled away. */}
          {blockedByHygiene && (
            <div className="-mx-5 -mb-5 mt-4 shrink-0 border-t border-hairline bg-canvas/40 px-5 py-3">
        ...
        >
          <div className="min-h-0 flex-1 overflow-y-auto pr-1">
            <div className="rounded-lg border border-hairline bg-canvas/60 px-3 py-2">
              ...
              <p className="mt-0.5 font-mono text-[16px] font-bold text-trust-active-soft">
                {mandate.token}
              </p>
            </div>
            ...
            {mandate.status === 'VOIDED' && mandate.voidReason && (
              <p className="mt-3 rounded-lg border border-trust-violation/50 bg-trust-violation/10 px-3 py-2 text-[14px] leading-relaxed text-trust-violation-soft">
                Mandate voided — {mandate.voidReason}
              </p>
            )}
          </div>

          {/* Pinned: the signature is the beat the demo turns on. */}
          <div className="-mx-5 -mb-5 mt-4 shrink-0 border-t border-hairline bg-canvas/40 px-5 py-3">
            <Button
              tone="active"
              size="lg"
