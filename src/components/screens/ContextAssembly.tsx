      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
            ...
            {CONTEXT_PILLS.map((pill) => {
              ...
              return (
                ...
                >
                  ...
                  <span className={cx('text-[11px] font-medium', t.text)}>{pill.label}</span>
                  <span className="font-mono text-[10px] text-ink-faint">{pill.freshness}%</span>
                </li>
              );
            })}
          </ul>
          ...
          <div className="mt-5 border-t border-hairline pt-4">
            ...
            {probeRun && dataRemediated && (
              <div className="mt-3 flex items-start gap-3 rounded-lg border border-trust-passed/50 bg-trust-passed/10 px-3 py-2.5">
                ...
                <div>
                  <p className="text-[11px] font-bold uppercase tracking-wider text-trust-passed">
            ...
            {blockedByHygiene && (
              <div className="mt-3 space-y-2.5">
                <div className="flex items-center gap-2 rounded-lg border-2 border-trust-hitl/60 bg-trust-hitl/10 px-3 py-2 shadow-glow-hitl">
                  ...
                  <p className="text-[11px] font-bold uppercase tracking-wider text-trust-hitl-soft">
                    ...
                    >
                      <div className="flex items-start gap-2.5">
                        ...
                        <div className="min-w-0">
                          <p className={cx('font-mono text-[11px] font-bold', t.text)}>{finding.artefact}</p>
                          <p className="mt-1 text-[11px] leading-relaxed text-ink-muted">{finding.detail}</p>
        ...
        >
          <div className="rounded-lg border border-hairline bg-canvas/60 px-3 py-2.5">
            <p className="text-[10px] font-bold uppercase tracking-[0.14em] text-ink-faint">Mandate token</p>
            <p className="mt-0.5 font-mono text-sm font-bold text-trust-active-soft">{mandate.token}</p>
          </div>
              ...
              {SCOPE_TREE.map((node) => {
                ...
                return (
                  ...
                  >
                    ...
                    {node.note && (
                      <span className="ml-auto shrink-0 font-mono text-[9px] uppercase tracking-wider text-ink-faint">
                        {node.note}
                      </span>
                    )}
                  </li>
          ...
          {mandate.status === 'VOIDED' && mandate.voidReason && (
            <p className="mt-4 rounded-lg border border-trust-violation/50 bg-trust-violation/10 px-3 py-2 text-[11px] leading-relaxed text-trust-violation-soft">
