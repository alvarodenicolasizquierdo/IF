    ...
    <div className="space-y-5">
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-[15px] text-ink-muted">
            What happens after go-live, which is where most of the cost of software actually lives.
          </p>
        </div>
        <StatusBadge label="Loop running" tone="active" pulse />
      </header>
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-5">
        ...
        <Panel
          ...
          eyebrow="Runs continuously after go-live"
          title="The loop"
        ...
        >
          <ol className="relative">
            {FCEE_STEPS.map((step, index) => {
              ...
              return (
                <li key={step.id} className="relative flex gap-4 pb-3.5 last:pb-0">
                  {!last && (
                    <span
                      aria-hidden
                      className={cx(
                        'absolute left-[11px] top-6 h-[calc(100%-12px)] w-px',
                        done ? 'bg-ink-faint/45' : 'bg-hairline',
                      )}
                    />
                  )}
                  <span className="relative z-10 mt-0.5 shrink-0">
                    {done ? (
                      <CheckCircle2 className="h-[22px] w-[22px] text-ink-faint" strokeWidth={2.2} />
                    ) : active ? (
                    ...
                    <div className="flex items-center gap-2">
                      <h3
                        className={cx(
                          ...
                          active ? 'text-trust-active-soft' : done ? 'text-ink-muted' : 'text-ink-faint',
                        )}
                      >
                        <Reveal label={step.name} detail={step.detail} side="right" muted={!active} />
                      </h3>
              ...
              <div className="flex items-start gap-2.5">
                ...
                <div>
                  ...
                  <p className="mt-1 text-[14px] leading-relaxed text-ink-muted">
                    The loop found an out-of-date tax API dependency in{' '}
